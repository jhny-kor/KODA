...
아카이브를 다운로드합니다. LFS blob은 포함하지 않으며, 압축 2 GB·해제 4 GB·파일
...
라이브러리 또는 전체 점검의 CycloneDX SBOM만 KODA-SBOM-Tracker로 자동 전송됩니다.
소스코드 전용 점검은 Tracker 전송과 결과 조회를 생략하고 `전송 대상 아님`으로 표시합니다.
소스 취약점 목록·증거 코드는 KODA와 GitLab에만 남기며, 전체 점검에서도 Tracker에는 SBOM만 전송합니다. GitLab API를 호출하는
...
예약 점검은 `설정 → 연동 설정`의 예약 점검 대상에서 서버, 원격 디렉터리,
제외 경로, 점검 규칙, KODA 프로젝트, GitLab 프로젝트와 결과 브랜치를 함께
지정합니다. SSH 계정은 읽기 전용으로 만들고 키와 검증된 `known_hosts`는
호스트의 예약 점검 SSH 디렉터리에 보관합니다. 관리자 화면의 활성화 기본값은
꺼짐입니다.

예약 점검 worker는 포털 데이터베이스나 수동 점검 입력·보고서 볼륨을 마운트하지
않습니다. worker와 포털은 인증된 제한 내부 API로 작업 임대, heartbeat, 결과 저장,
정리 확인만 주고받습니다. worker는 원격 파일을 전용 작업 디렉터리에 임시 수집하고
분석한 뒤 결과와 SBOM을 저장한 후 수집 파일·압축 해제본·분석용 복사본을 삭제합니다.
정리 확인 전에는 작업을 완료로 표시하지 않습니다. 프로세스 재시작이나 Tracker/
GitLab 일시 장애가 있어도 결과 전용 outbox로 재전송하며 원격 원본을 보관하지
않습니다.

관리자는 대상마다 한국시간 실행 시각과 매일·선택 요일·시간 간격을 지정합니다.
시간 간격은 1·2·3·4·6·8·12·24시간이며 지정 시각을 기준으로 반복합니다.
기존 대상은 매일 01:00을 유지합니다. 새로 저장하거나 변경한 시각 이전의 예약은
소급 실행하지 않습니다. 진행 중인 작업과 대기 작업을 먼저 마치며 놓친 회차를
여러 건 쌓지 않습니다. 수동 점검이 있으면 기존 우선순위를 적용합니다.

점검 대상은 원격 서버 디렉터리 또는 연결된 GitLab 저장소입니다. GitLab은 브랜치/
태그와 하위 디렉터리를 지정하며 빈 디렉터리는 저장소 전체입니다. 게시 저장소와
점검 원본 저장소는 구분합니다. GitLab 원본은 포털이 해당 실행의 커밋을 고정하여
인증된 내부 스트림으로 worker에 전달하고 포털 디스크에는 저장하지 않습니다.
원본 아카이브 전송과 압축 해제 제한은 전체 저장소에 적용되므로 큰 저장소는
설정된 용량 제한을 확인해야 합니다. LFS 객체와 서브모듈 내용은 수집하지 않습니다.
GitLab 아카이브 API는 [공식 문서](https://docs.gitlab.com/api/repositories/#retrieve-file-archive-from-a-repository)를 따릅니다.

첫 실행과 일요일은 전체 점검이고, 그 밖의 날은 변경분 점검입니다. 원격 서버는
경로·크기·수정시각, GitLab은 수집 파일의 SHA-256으로 변경 여부를 비교합니다. 규칙이나
대상 범위가 바뀌면 다음 작업은 전체 점검으로 전환됩니다. 예약 결과는 기존 결과
화면의 예약 점검 탭에서 수동 점검과 분리해 확인하고, 정리·Tracker·GitLab 게시
상태를 각각 봅니다.

예약 결과의 GitLab 게시 형식은 다음과 같습니다.

- 결과 브랜치: `koda/scheduled/<target-id>/<run-id>`
- 결과 JSON: `.koda/scheduled-results/<target-id>/<run-id>.json`
- 결과 JSON, Merge Request, 취약점 Issue는 동일 실행 재전송 시 기존 항목을 재사용
- 서버·디렉터리·실행 ID·전체/변경분·규칙 버전을 결과 출처로 기록
- 변경분에서 취약점이 보이지 않아도 기존 Issue를 자동 종료하지 않음

GitLab 게시 주체는 포털의 기존 게시 경로입니다. 예약 worker 컨테이너에는 GitLab
PAT나 Tracker 토큰을 전달하지 않으며, 토큰은 포털의 기존 secret 파일 참조로만
사용합니다. 예약 결과를 90일 보관한 뒤 KODA와 Tracker의 예약 결과만 정리하며,
GitLab에 게시한 파일·MR·Issue는 자동 삭제하지 않습니다.

Docker 배포 시 예약 worker는 `koda-schedule-worker`로 별도 실행되고 기본적으로
비활성입니다. 기본 제한은 CPU 1개, 메모리 4 GiB, 전송 5 MiB/s, 대상 사이 30초,
동시 작업 1개이며 worker 컨테이너의 CPU·메모리 상한을 넘도록 관리자 설정에서
확장할 수 없습니다. 중지·롤백은 `KODA_SCHEDULE_ENABLED=0`으로 예약을 비활성화한
뒤 worker를 중지하는 순서로 수행하고, 포털 데이터와 결과 볼륨은 보존합니다.

예약 worker는 원격 SSH 수집용 전용 Docker bridge와 포털 API용 기존 bridge에 연결합니다. 기존 포털의 외부 통신 제한은 변경하지 않습니다. 실행 중 포털에 SSH 키 마운트가 없다면 연결/접근 시험 화면을 쓰기 전에 다음 정상 포털 교체 시 `KODA_SCHEDULE_SSH_DIR` 마운트를 반영해야 합니다. worker 실행 자체는 전용 SSH 마운트를 사용합니다.

### 소스코드 결과와 Tracker 전송 분리 (2026-09-09)

이 정책은 수동 점검, GitLab 예약, 원격 서버 예약과 재전송에 공통 적용됩니다.
소스코드 전용 점검은 Tracker 없이 GitLab 결과 JSON·MR·취약점 Issue를 게시합니다.
라이브러리·전체 점검은 기존 SBOM 전송을 유지합니다. 기존 Tracker 데이터는 삭제하지 않습니다.

검증: 소스 전송 차단/직접 송신 차단/전체 점검 SBOM 본문/예약 정리 후 게시 테스트 4개,
기존 포털 회귀 65개, 스케줄 worker 15개, UI 2개 통과. Linux amd64 컨테이너에서도
새 경계 테스트 4개 통과. 실제 운영 서버 변경과 기존 폐쇄망 배포본 재포장은 하지 않았습니다.
